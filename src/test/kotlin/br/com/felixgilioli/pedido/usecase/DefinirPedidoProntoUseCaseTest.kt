...
class DefinirPedidoProntoUseCaseTest {

    private lateinit var pedidoRepository: PedidoRepository
    private lateinit var useCase: DefinirPedidoProntoUseCase

    @BeforeEach
    fun setUp() {
        pedidoRepository = mockk()
        useCase = DefinirPedidoProntoUseCase(pedidoRepository)
    }

    @Test
    fun definePedidoComoProntoQuandoEncontrado() {
        ...
        verify(exactly = 1) {
            pedidoRepository.save(withArg { pedidoSalvo ->
                ...
                assertEquals(StatusPedido.PRONTO, pedidoSalvo.status)
            })
        }
    }
